...
class GIFImage(object):
    ...
    def render(self, screen, pos, reverse=False):
        if self.running:
            if time.time() - self.ptime > self.frames[self.cur][1]:
                ...
                self.ptime = time.time()
        if reverse: screen.blit(pygame.transform.flip(self.frames[self.cur][0], True, False), pos)
        else:  screen.blit(self.frames[self.cur][0], pos)
